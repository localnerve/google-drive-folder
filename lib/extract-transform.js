 ...
 */
export async function convertFile(input) {
  return new Promise((resolve, reject) => {
    if (input.ext === '.md') {
      remark()
        .use(remarkHtml)
 ...
 */
export async function startObjectFlow(googDrive, objectStream, files) {
  let i = 0;
  try {
    let data;
 ...
 * @param {String} [outputDirectory] - Path to the output directory, defaults to '' (no fs output).
...
export async function extractTransform(
  ...
  try {
    ...
    throw handleError(
      ...
      `Credential file used: '${keyFile}'`,
      `Scopes used: '${scopes}'`,
      util.inspect(e, { depth: 6 })
    );
  }

  const objectStream = createObjectStream(convertFile);
  if (outputDirectory) {
  ...
  startObjectFlow(googDrive, objectStream, files);

...
import { createObjectStream, writeToDirectory } from './load.js';
...
export async function startObjectFlow (googDrive, files, objectStream, exportMimeMap) {
  ...
  objectStream.on('finish', () => {
    if (objectStream.customError) {
      objectStream.destroy(objectStream.customError);
    }
  });
  ...
  try {
    ...
    for (; i < files.length; i++) {
      ...
      if (objectStream.writableEnded || objectStream.errored) {
        break;
      }
      objectStream.write(data);
    }
  } catch (e) {
    objectStream.customError = handleError(
      e,
      `Failed downloading files at file '${files[i].name}'`,
      util.inspect(e, { depth: 6 })
    );
  } finally {
    if (!objectStream.writableEnded) {
      objectStream.end();
    }
  }
}
...
export async function extractTransform (folderId, userId, {
  ...
  if (outputDirectory) {
    objectStream.on('data', writeToDirectory.bind(null, outputDirectory, (e, msg) => {
      objectStream.customError = handleError(e, msg);
      objectStream.end();
    }));
  }
...
export { extractTransform as default }
